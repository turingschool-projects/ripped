...
context 'GitHelper' do
  ...
  describe 'after the API reponse' do
    it 'it updates a database object' do
      VCR.turn_off!
      WebMock.disable!
      folder = "ruby"
      ...
      exercise_items = {name: "Hello Test", content: "This is where the code goes", description: "This should be the new description"}
      ...
      expect(lessons.first.name).to eq(exercise_items[:name])
      expect(lessons.count).to eq(1)
      expect(lessons.first.description).to eq(exercise_items[:description])
    end
  end
end
